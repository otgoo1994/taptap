...
const query = {
  checkFbLogin: function(id) {
    return `SELECT id, name, phone, email, image, active, end_at, point, avg_wpm, record_wpm, lesson, avg_percent, rank, isCanInvite, cost  from users WHERE facebook = '${id}'`;
  },
  ...
  getUserInfo: function(id) {
    return `SELECT id, name, phone, image, email, active, end_at, point, avg_wpm, record_wpm, lesson, avg_percent, rank, isCanInvite, cost from users WHERE id = ${id}`
  },
  ...
  userLogin: function(email, password) {
    return `SELECT id, name, phone, email, image, active, end_at, point, avg_wpm, record_wpm, lesson, avg_percent, rank, isCanInvite, cost FROM users WHERE email = '${email}' AND password = '${sha256(password + process.env.SALT)}'`;
  },
  ...
  getUserList: function(name) {
    ...
    active, point, lesson, isCanInvite, cost FROM users 
