...
<template>
	<div style="padding-top: 5%;">
				...
				<el-table
					...
					<el-table-column
						...
						<template slot-scope="scope">
							<span style="margin-left: 10px">{{ scope.row.race_id }}</span>
						</template>
					</el-table-column>
					<el-table-column
						label="TITLE"
                    ...
                    <el-table-column
						label="CREATED">
						<template slot-scope="scope">
							<a-tag :color="scope.row.is_active ? 'green': 'orange'">{{ scope.row.is_active ? 'ACTIVE' : 'NOT ACTIVE' }}</a-tag>
						</template>
					</el-table-column>
					<el-table-column
						label="CREATED">
						<template slot-scope="scope">
							{{ new Date(scope.row.created_at).toLocaleDateString('en-us', { year:"numeric", month:"short", day:"numeric"}).toUpperCase() }}
						</template>
					</el-table-column>
					<el-table-column
						label="OPERATIONS">
						<template slot-scope="scope">
							<el-button
								size="mini" @click="handleEdit(scope.row)">Засварлах</el-button>
        ...
        <a-modal ref="modalRef" v-model="open" :wrap-style="{ overflow: 'hidden' }" @ok="handleOk">
			<el-select v-model="currentState" placeholder="Сонгоно уу" style="width: 100%; margin-top: 50px;">
                <el-option
...
<script>
	...
	export default ({
		...
		methods: {
            async handleOk() {
                await this.$_request('POST', this.$appUrl + '/admin/update-race', { state: this.currentState, race_id: this.current.race_id });
                this.open = false;
				this.getList();
            },
			pageChanged(page) {
			...
			async handleDelete(idx, row) {
				await this.$_request('POST', this.$appUrl + '/admin/delete-race-text', { id: row.race_id });
				this.getList();
			},
			handleEdit(row) {
				this.current = row;
                this.currentState = row.is_active;
                this.open = true;
			},
			async getList() {
				const data = await this.$_request('POST', this.$appUrl + '/admin/get-race-text-list');
