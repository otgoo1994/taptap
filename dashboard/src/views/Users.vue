...
<template>
        ...
        <a-card :bordered="false" class="header-solid h-full" :bodyStyle="{padding: 0,}">
          ...
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <!-- <span style="margin-left: 10px">{{ scope.row.status === 'PAID' ? new Date(scope.row.updated_at).toLocaleDateString() : '-'}}</span> -->
                <a-tag :color="scope.row.isCanInvite ? 'green' : 'orange'">{{ scope.row.isCanInvite ? 'SURE' : "NOT SURE" }}</a-tag>
              </template>
            </el-table-column>
            <el-table-column
              label="ACTIONS">
              <template slot-scope="scope">
                ...
                <template #overlay>
                  <a-menu>
                    ...
                    <a-menu-item @click="changeInviteStat(scope.row)">
                      Урилга илгээх эсэх
                    </a-menu-item>
    ...
    <a-modal v-model="modal.invite" title="Урилга илгээх эсэх">
      <a-select
        ref="select"
        style="width: 100%;"
        v-model="selected.invite"
      >
        <a-select-option value="true">Тийм</a-select-option>
        <a-select-option value="false">Үгүй</a-select-option>
      </a-select>

      <template #footer>
          <a-button key="back" @click="modal.date = false">Буцах</a-button>
          <a-button key="submit" type="primary" :loading="loading.invite" @click="reqChangeInviteStat">Үргэлжлүүлэх</a-button>
        </template>
    </a-modal>
	</div>
</template>
...
<script>
    ...
    computed: {},
    methods: {
      changeInviteStat(row) {
        this.selected.invite = row.isCanInvite ? 'true' : 'false';
      ...
      async reqChangeInviteStat() {
        this.loading.invite = true;
        const data = await this.$_request('POST', this.$appUrl + '/admin/update-user-invite-stat', { info: this.selected });
