...
export default {
    ...
    methods: {
        getNextLesson() {
            ...
            this.$axios({
                method: 'post',
                url: rts.$appUrl +'/lesson/next-lesson',
                data: {
                    level: this.lesson.lvl + 1
                }
            })
            .then(function(data) {
                if(data.data.data.type == 'boxed') {
                    rts.$router.push({name: 'boxed', params: {id: data.data.data.id}});
                } 
                else if(data.data.data.type == 'intro') {
                    rts.$router.push({name: 'intro', params: {id: data.data.data.id}});
                } 
                else if(data.data.data.type == 'review') {
                    rts.$router.push({name: 'review', params: {id: data.data.data.id}});
                } 
                else if(data.data.data.type == 'hold') {
                    rts.$router.push({name: 'hold', params: {id: data.data.data.id}});
                }
            });
        },
        showResultDialog() {
        ...
        resetParams() {
            ...
            // this.chart = { data: null, keyword: null, wpm: [], score: 0, accuracy: 0, characters: 0 }
            this.chart.wpm = [];
