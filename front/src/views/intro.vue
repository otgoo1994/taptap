        ...
        <div class="intro-x">
            <div :style="{'padding-top': '50px', 'position': 'relative'}" align="center" @click="setFocus">
                ...
                <div align="left" class="intro-text mt-5" style="display: flex;">
                    ...
                    <div :style="{'margin-top': '5px', 'padding-left': '20px', color: '#646669'}">үсгэн дээр дарна уу.</div>
                </div>
...
export default {
    ...
    methods: {
        ...
        async finishGame() {
            ...
            var wpmPoint = 100;
            this.counter.score = wpmPoint + diff_acc;
            this.chart.accuracy = accuracy;
            this.chart.score = this.counter.score;
            this.chart.characters = typed;

            const data = await this.$_request('POST', this.$appUrl +'/lesson/update-user-lesson', {lessonId: this.lesson.id, wpm, accuracy, score: this.counter.score, level: this.lesson.lvl});
            if (Number.isInteger(data)) { 
                if (data === 402) { this.$router.push('/price'); return; }
                return;
            }
